...
class ProcessModelTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testAccessNodesAndTransitions()
	{
		...
		$model = $builder->build();
		$this->assertEquals([$s1], $model->findStartNodes());
		$this->assertEquals([$s1, $n1, $n2], $model->findNodes());
		$this->assertEquals([$t1, $t2], $model->findTransitions());
		$this->assertEquals([$t1, $t2], $model->findOutgoingTransitions($s1));
		$this->assertEquals([$t2], $model->findIncomingTransitions($n2));
	...
	{
		call_user_func([(new ProcessBuilder())->build(), $methodName], $arg);
	}
}
