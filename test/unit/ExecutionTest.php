...
class ExecutionTest extends ProcessTestCase
{
	...
	public function testTerminationBlocksMethodCalls($methodName, array $args = [])
	{
		...
		call_user_func_array([$execution, $methodName], $args);
	}
	...
	public function testVarScopeLocal()
	{
		...
		$this->assertEquals(['foo' => 'bar'], $exec->getVariablesLocal());
		$this->assertEquals(['foo' => 'bar'], $child->getVariablesLocal());
	...
	public function testVarScopeConcurrent()
	{
		...
		$this->assertEquals(['foo' => 'bar'], $exec->getVariables());
		$this->assertEquals(['foo' => 'bar'], $exec->getVariablesLocal());
		$this->assertEquals(['foo' => 'bar'], $child->getVariables());
		$this->assertEquals(['foo' => 'bar'], $child->getVariablesLocal());
